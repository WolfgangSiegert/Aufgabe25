...
double Male::calculateBloodAlc(std::vector<std::string> paramBevs,std::vector<double> paramBevsAlc){
    ...
    for (size_t i = 0; i < paramBevsAlc.size(); i++) // Loop fuer Ausagabe der Konsumierten Getraenke und Mengen auf der Konsole und Aufrechnung des konsumierten Alkoholvoluens insgesamt
    {
        if (qtyPerBev.at(i) > 0){
